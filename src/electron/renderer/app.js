...
function renderLimits() {
  ...
  const hideUnconfigured = state.settings?.hideUnconfiguredLimitProviders !== false;
  const visibleProviderEntries = new Map();
  for (const { id } of orderedProviders) {
    ...
    const shown = providerEntries.filter((provider) => (
      limitProviderPresentationApi.shouldShowLimitProviderRow(provider, { hideUnconfigured })
    ));
    if (shown.length) visibleProviderEntries.set(id, shown);
  }
  const rows = orderedProviders.filter(({ id }) => visibleProviderEntries.has(id));
...
els.hideUnconfiguredLimitProvidersInput.addEventListener('change', async () => {
  await saveSettings({ hideUnconfiguredLimitProviders: els.hideUnconfiguredLimitProvidersInput.checked });
  renderLimits();
});
