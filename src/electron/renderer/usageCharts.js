...
(function exposeUsageCharts(root, factory) {
  ...
  const clientColors = {
    ...
    // Mavis (MiniMax Code) — locally-parsed pi-agent SQLite, shares the
    // MiniMax brand with minimax. MiniMax brand pink so the row stays on-
    // palette with the tokscale-side `minimax` entry.
    mavis: '#f23f5d',
    default: '#6ab4f0'
  };
