...
- **側邊欄（macOS 與 Windows）**：貼在螢幕邊緣的窄長欄，游標推到邊緣才滑出，也可以設為永遠顯示。以圓環顯示各額度提供者，並可加入用量數字（今日、本星期、最近 7／30 日、本月、總計、即時速率）；滑鼠停在項目上會展開卡片，列出每個帳號的額度、最近會話與 token 用量。macOS 上弧形欄與卡片使用真正的毛玻璃。可在設定中挑選、排序與設定項目，也能從選單列或系統匣開關
