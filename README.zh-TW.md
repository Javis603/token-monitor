...
活動熱力圖與趨勢儀錶板，是由各工具仍留在磁碟上的會話檔建構的。**Claude Code 預設會清除 30 天前的 transcript**（`cleanupPeriodDays`）。開啟**保留已刪除會話用量**（設定 → 採集）後，Token Monitor 除了保留今日／本月／全部的會話總量，也會保存已觀測到的每日工具／模型資料，涵蓋熱力圖目前 370 天的滾動視窗。來源日後清理檔案時，已觀測的日期不會再消失；但在 Token Monitor 首次觀測前就已刪除的資料，無法由這份封存記錄救回。
