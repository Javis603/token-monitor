...
활동 히트맵과 트렌드 대시보드는 각 도구가 디스크에 남겨 둔 세션 파일로 만들어집니다. **Claude Code는 기본적으로 30일이 지난 트랜스크립트를 삭제합니다**(`cleanupPeriodDays`). **삭제된 세션 사용량 유지**(설정 → 수집)를 켜면 Token Monitor는 오늘/이번 달/전체 세션 합계와 함께 히트맵의 370일 롤링 기간에서 이미 관측한 일별 도구/모델 정보도 보존합니다. 이후 원본 파일이 정리되어도 관측된 날짜는 사라지지 않지만, Token Monitor가 처음 관측하기 전에 이미 삭제된 데이터는 이 아카이브로 복구할 수 없습니다.
