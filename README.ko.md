...
- **가장자리 도크 (macOS·Windows)** — 화면 가장자리에 붙는 슬림한 레일로, 포인터를 가장자리로 밀면 나타나고 항상 표시로 둘 수도 있습니다. 한도 제공자를 링으로 보여 주고 사용량(오늘, 이번 주, 최근 7/30일, 이번 달, 전체, 실시간 속도)도 추가할 수 있으며, 항목에 마우스를 올리면 계정별 한도·최근 세션·토큰 사용량 카드가 열립니다. macOS에서는 레일과 카드에 실제 반투명 유리 효과를 사용합니다. 설정에서 항목을 고르고 정렬·구성할 수 있고 메뉴 막대나 시스템 트레이에서 켜고 끌 수 있습니다
