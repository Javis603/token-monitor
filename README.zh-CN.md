...
- **AI 工具额度检测**：涵盖 Claude Code、Codex、Cursor、OpenRouter、第三方 API、GLM、Kimi 等 23+ 家提供方的 session、每日、每周、账单与 credits 窗口，支持多个 OpenRouter／第三方 profile，以及 DeepSeek 预付余额与消费。Kimi 的网页凭据使用即轮换：一旦 Token Monitor 接管了一对令牌（桌面版会话或粘贴的 refresh_token），再用同一账号的网页控制台会在下次刷新时登出该浏览器会话
