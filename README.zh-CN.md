...
一款桌面小部件，实时显示 Claude Code、Codex、Cursor、GitHub Copilot、Cherry Studio 等 31+ 种 AI 编程工具的 Token 用量与 AI 工具额度，具备实时多设备同步与历史使用趋势功能，并支持按工具、设备、模型、session 或项目分项显示。
...
| <img src=".github/assets/tools-icon/workbuddy.png" width="28" alt="WorkBuddy" /> | WorkBuddy | `~/.workbuddy/projects/`、`~/.workbuddy/workbuddy.db`；在设置中明确启用本地应用监测，再在 WorkBuddy 中登录后读取账单余额（支持 macOS/Windows，Linux 不支持） | ✅ | ✅ | — |
...
- **AI 工具额度检测**：涵盖 Claude Code、Codex、Cursor、OpenRouter、第三方 API、GLM、Kimi 等 20+ 家提供方的 session、每周、账单与 credits 窗口，支持 WorkBuddy 读取本机应用登录状态并查询账单余额、多个 OpenRouter／第三方 profile，以及 DeepSeek 预付余额与消费
