...
- **侧边栏（macOS 与 Windows）**：贴在屏幕边缘的窄长栏，鼠标推到边缘才滑出，也可以设为始终显示。以圆环显示各额度提供者，并可加入用量数字（今日、本周、最近 7／30 天、本月、总计、实时速率）；鼠标停在项目上会展开卡片，列出每个账号的额度、最近会话与 token 用量。macOS 上弧形栏与卡片使用真正的毛玻璃。可在设置中挑选、排序与配置项目，也能从菜单栏或系统托盘开关
