...
活动热力图与趋势仪表板，是由各工具仍留在磁盘上的会话文件构建的。**Claude Code 默认会清除 30 天前的 transcript**（`cleanupPeriodDays`）。开启**保留已删除会话用量**（设置 → 采集）后，Token Monitor 除了保留今日／本月／全部的会话总量，也会保存已观测到的每日工具／模型数据，覆盖热力图当前 370 天的滚动窗口。来源日后清理文件时，已观测的日期不会再消失；但在 Token Monitor 首次观测前就已删除的数据，无法由这份归档记录找回。
