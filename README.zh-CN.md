...
| <img src=".github/assets/tools-icon/dsh.png" width="28" alt="DeepSeek Harness" /> | DeepSeek Harness | `<DSH_HOME>/sessions/`（zstd JSONL 日志；`$DSH_HOME` 环境变量或 `~/.dsh`） | ✅ | — | — |
...
上表为默认路径。Token Monitor 与 Tokscale 遵循相同的环境变量覆盖：`~/.local/share/` 下的路径跟随 `$XDG_DATA_HOME`，各工具另有 `$CODEX_HOME`、`$GROK_HOME`、`$HERMES_HOME`、`$KIMI_CODE_HOME`、`$REASONIX_STATE_HOME`、`$REASONIX_HOME`、`$DSH_HOME` 以及 `$CLINE_*` 系列。
