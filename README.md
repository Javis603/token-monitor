...
- **Edge Dock (macOS & Windows)** — a slim rail docked to the screen edge that hides until you push the pointer against the edge, or stays visible. It shows quota rings for your limit providers plus usage figures (today, this week, last 7 / 30 days, this month, total, live token rate); hover an item for a card with every account's limits, recent sessions, and token usage. On macOS the sculpted rail and cards use real translucent glass. Choose, reorder, and configure items in Settings, and toggle it from the menu bar or the system tray
