...
Qoder CN token usage is read from the app's local data, not an API — enable it in Settings → tools (opt-in, off by default). Qoder CN moved its storage backend in ~2026-09: builds now persist Claude-compatible JSONL transcripts under `~/.qoder-cn/projects/**/*.jsonl` (a `<sessionId>.jsonl` per session plus `<sessionId>/subagents/agent-*.jsonl` side-chains), while older builds wrote a SQLite database. The adapter reads both, so a machine with either layout reports usage. The transcript tree is auto-detected at `~/.qoder-cn/projects` (overridable with `TOKEN_MONITOR_QODER_CN_PROJECTS_PATH`); the legacy database is auto-detected per platform: macOS `~/Library/Application Support/QoderCN/SharedClientCache/cache/db/local.db`, Windows `%APPDATA%\QoderCN\SharedClientCache\cache\db\local.db`, Linux `~/.config/QoderCN/SharedClientCache/cache/db/local.db` (overridable with `TOKEN_MONITOR_QODER_CN_DB_PATH`).

This is an advanced local integration: the JSONL path needs no extra runtime, but reading the legacy database needs a `sqlite3` CLI on PATH or a Node runtime with unflagged `node:sqlite` (Node ≥ 23.4; the Electron widget may need the CLI). Read failures are logged, and an existing complete snapshot is retained instead of being replaced with zero usage. Costs are estimated from the models.dev catalog for each mapped model; the adapter may break if Qoder changes its storage format.

Current CN builds report `input_tokens: 0` for internal (plan-billed) models — those rows carry only `credits` plus a `context_usage_ratio`. Setting `TOKEN_MONITOR_QODER_CN_ESTIMATE_PROMPT_TOKENS=1` reconstructs their prompt-side token counts as `ratio × the model's published context window` (a 1M window is built in for the known internal codes; extend it with `TOKEN_MONITOR_QODER_CN_MODEL_WINDOWS=code=tokens,...`). Routing tiers (`auto` etc.) and unknown codes stay at zero — their denominator is not published and fabricated tokens are worse than none. Reconstructed rows count toward token totals but never toward cost estimates, and output tokens stay unrecoverable (the ratio only covers context occupancy).
</details>
