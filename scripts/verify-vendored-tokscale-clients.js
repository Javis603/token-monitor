...
const { DEFAULT_CLIENTS } = require(path.join(__dirname, '..', 'src', 'shared', 'clientTracking'));
...
const LOCALLY_PARSED_CLIENTS = new Set(['proma', 'qodercn']);
...
function main() {
  ...
  const unsupported = DEFAULT_CLIENTS.split(',').filter(
  ...
  if (unsupported.length > 0) {
    throw new Error(
      `Vendored tokscale (${key}) does not recognize these DEFAULT_CLIENTS entries: ${unsupported.join(', ')}. ` +
        'Either the vendor pin needs updating, or these clients need to be parsed locally (add to LOCALLY_PARSED_CLIENTS) ' +
        'or removed from DEFAULT_CLIENTS.'
    );
  }

  console.log(`Verified vendored tokscale (${key}): all ${DEFAULT_CLIENTS.split(',').length} default clients are supported (tokscale-native or locally parsed).`);
}
