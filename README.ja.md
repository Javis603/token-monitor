...
- **サブスクリプション記録** — 各 AI アカウントに実際に支払っている金額を手動で記録し、そのアカウントのプランラベルにカーソルを合わせると、価格、更新日または終了日、利用期間、当月の使用量とプラン料金の比較が表示されます。定期プランとチャージ履歴の両方に対応し、ハブを設定すると接続中のすべての端末で共有されます
