...
- **エッジドック（macOS・Windows）** — 画面の端に収まるスリムなレール。ポインタを端に押し当てると表示され、常に表示することもできます。上限プロバイダをリングで示し、使用量（今日・今週・過去 7／30 日・今月・合計・リアルタイム速度）も追加可能。項目にホバーすると、各アカウントの上限・最近のセッション・トークン使用量をまとめたカードを表示します。macOS ではレールとカードに本物の半透明ガラスを使用。設定で項目の選択・並べ替え・調整ができ、メニューバーまたはシステムトレイからオン／オフも切り替えられます
