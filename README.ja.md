...
- **AI ツール制限検出** — Claude Code、Codex、Cursor、OpenRouter、サードパーティAPI、GLM、Kimi など 24+ プロバイダーの session/daily/weekly/billing/credits、複数の OpenRouter／サードパーティプロファイル、残高型アカウント（Claude クレジット、DeepSeek のプリペイド残高と使用履歴、サードパーティ残高）。Kimi のウェブ資格情報は使用のたびにローテーションされます。Token Monitor がトークンペアを保持すると（デスクトップアプリのセッションまたは貼り付けた refresh_token）、同じアカウントのウェブコンソールは次回の更新でサインアウトされます
