...
4. watches the per-client data directories from `watchClientRootsForClients()` with chokidar (native events on every platform and in every mode; `resolveWatchUsePolling()` owns that default so entry points can't drift, and `TOKEN_MONITOR_WATCH_POLLING` overrides it in both directions) and debounces refreshes by `watchDebounceMs` (no cooldown — the product promises 3–5 s updates; mid-tick watch events re-arm the debounce timer instead of coalescing). A live watch tick maps the changed path back to its client and scans only that client's `--today` partition; smart mode retains the same mapping plus any tracked self-synced clients until its next active interval. Multiple clients are unioned, while unknown/unattributed paths fall back to an all-client `--today` scan. The cursor/antigravity tokscale cache dirs are deliberately *not* watched — only our own `maybeSync*` calls write them, so watching them re-triggers forever — and those syncs are gated + throttled (`SYNC_MIN_INTERVAL_MS`). Antigravity's *source* roots (`selfSyncSourceRootsForClients()`) are a different matter and are watched: tokscale only reads them, writing solely into its own cache dir, so an event there cannot close the loop. `watchIgnoreMatcher()` prunes those roots to the paths a turn actually touches, and takes `brain/` one level deep only — its per-session children are plans, uploads and screenshots, hundreds of directories per home for a handful of writes a week, and on Linux that is an inotify descriptor each. Two selections shorten the throttle for a named client, and they are not interchangeable. `forceSelfSync` (`true` for the manual refresh button, `[client]` for a scoped action such as a Cursor sign-in) waits for nothing — the `{ force: true }` refresh that every settings and account flow performs deliberately does not set it, or each one would pay for both sync subprocesses. `sourceSelfSync` is set by a watch event on those source roots and drops the client to `SYNC_SOURCE_EVENT_MIN_INTERVAL_MS` instead; a client still inside that floor stays pending and arms a catch-up tick for the moment it clears, because a source event means the cache is stale whether or not another event follows — dropping it would strand a turn that ended inside the floor until the fallback interval. That catch-up re-arms rather than queueing when it comes due mid-tick, for the same reason `scheduleTick` does: the coalesce state carries the sync selections but not `targetClients`, so folding in would widen a one-client scan into an all-client one. A `forceSelfSync` tick cancels the pending catch-up for the clients it names as it starts, since a forced sync re-reads the source anyway and the deferred one would otherwise spend a second full sync on a change already collected. `maybeSync*` resolves on every outcome so a stuck sync cannot hold a tick open, which makes `onFailure` the only signal that one did not land; a tick restores the source events it consumed for any client that reports failure, but never invents one for a plain cadence sync, or a wedged language server would turn the idle cadence into a retry loop. The short floor is conditional on the last attempt having worked (`sourceSyncFloorMs`): once a sync fails, that client sits on the idle cadence until one succeeds, since nothing upstream separates a transient failure from a permanent one and the restored event drives the next attempt. Keep that a property of the client rather than a longer timer — the floor is read by the drain, the catch-up arm and the sync decision, and backing off only the timer still let an unrelated client's watch event drain the failed one on the fast floor, consuming a pending event for a sync that would then be refused. A manual refresh overrides it outright. `stop()` cannot cancel a sync already in flight, so completions carry an attempt number and only the newest one for a kind may write that state — a collector rebuilt by a settings change must not inherit the previous one's verdict. The scheduling paths that remove from the pending set — the drain, the acknowledgement, the failure restore — re-arm the shared catch-up from the whole set rather than from one client's delay, for the same reason the floor is one function: separate copies of the deadline are what let this state machine diverge. A watcher event only adds, and deliberately does not re-arm: live mode drains it on the debounce that same event schedules, and smart mode is supposed to wait for its interval. It is a shorter floor and not the absence of one on purpose: `tokscale antigravity sync` re-fetches over RPC and rewrites *every* known session artifact on each run (the manifest carries `artifactHash`/`lastModifiedMs` but upstream never short-circuits on them), on top of a `ps` over the whole process table and an `lsof` per candidate, so its cost scales with the user's session count while the per-turn source file is a SQLite WAL that churns for the whole turn. Note the roots feeding `sourceSelfSync` are a strict subset of that client's watch roots: the parse-local `antigravity-cli` dir shares the umbrella client id, so it targets the same scan but must not buy an IDE sync it has no use for. Keep both selections distinct from a tick's `todayOnly`: the coalesced replay in `runTick` derives its scan scope from the pending ticks themselves, so that forcing a sync cannot downgrade a manual full scan into a warm one. Kernel watch-descriptor exhaustion (`ENOSPC`/`EMFILE`/`ENFILE` — a per-user budget shared with editors, hit first on Linux) arrives as an async watcher error and would otherwise just stop event delivery silently, so it rebuilds the watcher on polling; that fallback is deliberately sticky for the process, because a later rebuild would only rediscover the same exhausted budget.
