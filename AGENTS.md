...
Balance-style quotas use two orthogonal shared-schema markers. `windows[].metric === 'credits'` identifies a monetary/provider-credit balance whose headline uses `remaining` plus `currency`; `windows[].displayRole === 'balance'` identifies any absolute-balance presentation, including non-monetary provider units such as WorkBuddy. `windows[].unit` carries the provider unit (`credits` for WorkBuddy) when the value is not currency. `src/shared/limitBalanceDisplay.js` is the single formatting/derivation entry point shared by Home, the tray and the limits page — key off these schema markers, never a provider whitelist. The meter percentage for a top-up balance (`amount / (amount + monthSpend)`) is a **display-layer derivation** and is deliberately kept out of the wire shape; don't push it back into a collector. Keep `src/shared/limits.js`, `docs/API.md`, and the generated Worker normalization aligned when these markers change.
