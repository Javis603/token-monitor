...
- Limits providers declare their id and order once, in `LIMIT_PROVIDER_IDS` (`src/shared/limitProviders.js`), and their display name once, in `src/shared/limitProviderLabels.js`. The renderer's `LIMIT_PROVIDERS` and the mac widget snapshot both derive from those, so a new provider still needs account settings when applicable, tray artwork, a `.row-icon-<id>` mask, an explicit case in the Swift `WidgetFormat.provider` mirror, and every README table. `LIMIT_PROVIDER_IDS` defines the new-install order; a changed default must not overwrite a saved custom order.
- Labels live outside `limitProviders.js` on purpose: that file is part of the portable Hub core hashed into `hubBuildRegistry.json`, so folding desktop-only presentation into it would make every renamed provider bump the core build id and tell self-hosted Hubs to redeploy for a change their runtime cannot observe. Same reason `limitProviderLabels.js` stays out of `WORKER_SHARED_MODULES`.
