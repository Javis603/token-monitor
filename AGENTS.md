...
Subprocess lifecycle invariant: sending `SIGTERM` only requests cancellation. A replacement tick stays behind the old usage runtime's quiescence barrier until every usage child emits `close`; a child that ignores the request is escalated to `SIGKILL` after the shared grace period.
