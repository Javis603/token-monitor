...
The app, headless-agent, and packaging scripts explicitly run `ensure:tokscale` before execution. The four vendored targets use the pinned binary from `scripts/vendor/tokscale.json`; source platforms without a vendored asset keep the npm binary and cache a capability filter for unsupported clients. `npm install`, `npm ci`, `npm run hub`, lint, test, and verify do not download the vendored asset. Setting the manifest's `mode` to `"upstream"` (alongside bumping the `tokscale` npm dependency past the pinned commit) only changes binary provenance: `ensure:tokscale` stops downloading/replacing the binary (it still fails closed if a packaging target's npm platform package isn't installed), while both `verify-vendored-tokscale*.js` gates keep running — now against the plain npm-installed binary — so CI still fails loudly if it doesn't actually cover `DEFAULT_CLIENTS` or regressed DSH parsing. None of this wiring gets removed — flipping back to `"override"` for a future upstream lag is just editing the manifest again, not re-adding scripts, package.json wiring, or workflow steps.
