...
Provider identity lives in **one** place: `LIMIT_PROVIDER_CATALOG` in `src/shared/limitProviders.js`. The catalog order is the new-install order; a changed default must not overwrite a saved custom order. Everything below is a separate hand-wired registration point that must agree with that id.
...
| Collection | an entry in `providerFetchers()` (`src/shared/limitCollector.js`) plus the implementation, in that file or a `src/shared/*Limits.js` |
...
| Capability tags & source labels | `CAPABILITY_TAGS` and `PROVIDER_SOURCE_LABELS` in `src/electron/renderer/limitProviderPresentation.js`. Neither errors when missing: the settings row simply renders without the tags that say how the provider is collected, and the source label falls back to the generic one |
...
| Icon assets | `assets/icons/<id>.svg` + `.github/assets/tools-icon/<id>.png`. Tray artwork follows by convention; `SPECIAL_ICON_SOURCES` (`trayProviderIcons.js`) is only for a menubar-optimized or shared file |
...
Most of that table is now asserted from the catalog, so a provider that misses one of those points fails CI rather than shipping — `grep LIMIT_PROVIDER_IDS tests/` shows which. Two are not, and fail silently: the manual-panel shapes, where a missing registration and a correct omission look identical from the id lists alone, and the i18n keys.
