...
3. funnels output through the shared extractors in `src/shared/usage.js`, which defensively deep-walk tokscale's JSON shape (they never assume a fixed layout — that's why `tokenValue`/`detectClient` accept many key spellings). Before that, the scan and graph runners are wrapped (`withScanPricingFallback` / `withGraphPricingFallback`) so a row tokscale returned at `cost: 0` for a first-party model its pricing catalogs do not carry yet is re-costed from the bundled list rates in `src/shared/modelPricingFallback.js`; `custom-pricing.json` wins, the table is inert once a catalog prices the row, and its revision is part of `configFingerprint` so an edit retires persisted anchors — read the module header before adding a row;
