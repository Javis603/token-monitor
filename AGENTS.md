...
Subprocess lifecycle invariant: sending `SIGTERM` only requests termination. Aborts, command timeouts, and pipe failures keep the operation pending until the usage child emits `close`, so a replacement tick stays behind the old runtime's quiescence barrier; a child that ignores the request is escalated to `SIGKILL` after the shared grace period.
