...
Propagation is by version stamp, not by shipping the list: an accepted `PUT` broadcasts stats the way an ingest does, and every stats frame carries `subscriptionsUpdatedAt`. A device refreshes only when that disagrees with the copy it holds, so the steady state costs nothing and an edit still lands on the other devices in seconds rather than on their next poll — which is five minutes apart precisely while the stream is up (`restartTimer()`). Two traps: the Worker's `/api/public/stats` spreads the rest of `getStats()`, so anything added there is published unless it is destructured out; and a missing stamp must read as "no news", since that is also what a local collector's own stats and a hub older than the field look like.
