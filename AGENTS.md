...
`limitsRefreshMode` selects the scheduling policy and is deliberately a separate setting from `limitsRefreshMs`: `fixed` (the default) keeps the chosen interval exactly as it always behaved, while `adaptive` substitutes its own baseline from `limitsBurnRate.js` so the stored interval survives a round trip through adaptive and back, and no consumer doing arithmetic on `limitsRefreshMs` has to handle a sentinel. Adaptive additionally runs an urgency timer that inserts an earlier provider-scoped probe when a window's measured burn rate says it is close to exhaustion (`remaining% / burnRate / 4`, floored at 60 s, never longer than the baseline). The trigger is time-to-exhaustion, not a "remaining < N%" threshold, which is wrong at both ends: a threshold polls all night for a low but idle quota, and stays slow for one being consumed fast from a comfortable level. That ratio is a control target, **not** an invariant — the rate is measured between two samples and says nothing about acceleration inside the next gap.

Three constraints in that path are load-bearing. A dispatched urgency probe stays in `inFlight` until it settles, because a lane is latest-wins and rescheduling a scope whose probe is still running aborts it: a provider slower than the floor would otherwise publish nothing and emit only cancelled requests. `burn-rate` stays out of `COOLDOWN_BYPASS_REASONS`, so a backing-off lane defers it like any other scheduled reason, and there is deliberately no circuit breaker on top of that — a 429 is transient traffic control, not proof that a provider is unsuited to adaptive polling. And local token usage is deliberately not a trigger: a client's tokens may be billed to a third-party API key or endpoint rather than the subscription being metered, and quota can equally be consumed from another device or the web, so the correlation breaks in both directions. Separately from all of this, `limitResetBoundary.js` re-probes just after a window resets, in every mode.
