...
The Billing action's `resetsInDays` does not match the Console's credit-grant expiry date and must not be mapped to the balance window's `resetsAt`. The provider does not collect grant expirations, so it displays no balance reset or expiry countdown. A single grant labeled "Monthly credit" does not establish that another grant will arrive next month.
