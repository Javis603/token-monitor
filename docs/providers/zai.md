...
| ZCode Start/Weekend plans | ZCode login: the credential store's account JWT (3.12.3+), or the provider entry's mirror (3.11.x) | `zcode.z.ai/api/v1/zcode-plan/billing/balance` | model-aggregated `daily`/`billing` with `limitId` |
...
- A **start-plan/account JWT** calls billing. A **coding-plan key** calls quota. These are different selections and credentials, not one JWT that is assumed to work on both endpoints. Discovery resolves both from the credential store when one exists — `zcodejwttoken` for billing, and the account's own key (`account-provider:coding-plan:account:<family>-<kind>:account:<profile user id>:api-key`) for quota — reading it on every call, in memory, and falling back to the provider entry's plaintext `options.apiKey` mirror when the store is absent (3.11.x) or cannot name the entry. Preferring the store is about identity, not just freshness: after an account switch the mirror still names the *previous* account, so a mirror-first read would attribute another account's quota to the current login. The decrypted value never enters Token Monitor's credential store, the renderer, or a log; every failure (missing store, malformed envelope, another machine's key) degrades silently to the mirror, and the OS keychain is not read. This store read is the one boundary that departs from the note's original "never decrypts `credentials.json`" claim; the maintainer approved it in #718, scoped to ZCode's own app data, an exact per-account entry reconstructed from the profile `user_id` (never searched), and a silent fallback to the mirror on any failure.
- ZCode 3.12.3 keeps its live credentials in `credentials.json` and stopped rewriting the plaintext mirrors in `config.json` that this discovery reads, so a mirror can still carry the previously signed-in account — or, on a fresh 3.12.3 install, never have been written at all. The mirrors stay acceptable to their endpoints, so an account mismatch is silent rather than an auth error. A pasted API key does not go through discovery and is unaffected; the credential-store read closes this gap under the boundary approved in #718.
