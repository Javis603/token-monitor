...
Loaded spend entries discard invalid day/value pairs and repair malformed totals and timestamps before aggregation, persisting the repair even if the cumulative total has not changed. Invalid store/account container shapes reinitialize safely. A failed write is best-effort. The store remains a device-local read-modify-write file: concurrent writers can lose history; it is not an atomic transaction or guaranteed self-healing after races.
