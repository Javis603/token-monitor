...
The usage events carry conversation ids but no names. Session titles come from the local Cursor desktop `composerHeaders` table, joined by conversation id. The reader opens the database read-only and refreshes its title cache when the database or WAL changes. Sessions without a local header retain the normal client/model fallback.
