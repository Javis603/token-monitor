...
- **Why the account cookie stops being accepted.** Not established, and deliberately not pursued: a refused exchange is a user-visible `auth-expired` whose remedy is one re-login, so the lifetime and the exact cause do not change anything downstream. What is on record is the shape of the event — it was **account-wide rather than per-request** (the cookie stayed in the partition while MiMo Desktop itself fell back to its login screen) and it produced exactly the signatures in §3. One candidate is worth remembering for whoever debugs this next: an invalid `passToken` was deliberately sent to the SSO twice while establishing the rejected-cookie signature, and a security-driven revocation is a plausible response. Do not repeat that probe casually.
...
- **Console lane.** The existing `mimoManagedAccounts` cookie paste (and its settings panel) stays exactly as it is — minting is additive, not a replacement. One consequence has to be handled: a minted session and a pasted session for the **same account must collapse onto one identity**. `mimoAccountKey` already prefers an explicit `account.userId` and otherwise reads the `userId` cookie, so the minted path — whose freshly minted cookies do **not** include `userId` — has to pass the id `/userProfile` returns explicitly, or the same account lands on two rows.
