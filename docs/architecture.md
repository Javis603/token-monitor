...
`src/shared/collector.js` owns every tokscale scan and the binary resolution behind it; self-sync spawns take its resolver rather than repeating it. `src/shared/clientSources.js` resolves host source roots (including exact-file watch parents and Copilot's canonical exporter path); the collector projects those roots into watch candidates, source checks and diagnostics. Keep the exporter root, ignore policy and attribution on the same canonical path helper, or a symlink/Windows short path can make the watch and its attribution disagree.
