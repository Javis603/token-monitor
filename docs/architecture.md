...
- A missing identity in a mixed provider result is transient and retains its last good data. A local discovery source that knows an identity was removed returns an internal `{ provider, accountKey, removed: true }` marker; `LimitsRuntime` consumes it before normalization, and one-shot collection drops it, so it never enters the device or Hub wire shape.
