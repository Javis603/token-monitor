...
History daily/monthly rows and the lifetime summary may additionally carry `clientModelCosts: { [client]: { [model]: costUsd } }`. This is raw, already-priced joint attribution for local display preferences, not a changed billing total. Mixed legacy histories or size-budget omissions set `clientModelCostsIncomplete: true`; missing attribution must never be interpreted as zero cost. Compact history previews retain these cost-only maps when available. The producer may omit these optional maps before evicting session/project identities to fit the ingest size limit. Desktop `usageCostRules` remain local settings and are never applied to ingested or stored raw records.
