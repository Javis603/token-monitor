...
`limits.providers[].provider` is one of `claude`, `codex`, `cursor`, `antigravity`, `opencode`, `openrouter`, `deepseek`, `minimax`, `mimo`, `grok`, `copilot`, `kiro`, `zai`, `zaiteam`, `volcengine`, `qoder`, `workbuddy`, `kimi`, `ollama`, or `thirdparty`.
...
`limits.providers[].source` is one of `oauth`, `cli`, `web`, `rpc`, `local`, or `api`; `local` means the value was read from an app-owned local store or capability (including WorkBuddy's local app session), `web` means a browser/session cookie-backed web endpoint (Cursor, OpenCode web accounts, Qoder, MiMo, Kimi membership, Ollama), and `api` means a provider HTTP API authenticated by an API key, access token, or AK/SK credentials (OpenRouter, DeepSeek, Minimax, Copilot, GLM/Z.ai, Volcengine, Kimi Code, and third-party adapters). WorkBuddy's billing request is made in the Electron main process using the session owned by the local WorkBuddy app only after the user explicitly enables Local App monitoring; raw credentials and response bodies are never placed on the wire. Electron local-app session monitoring is supported on macOS and Windows; Linux intentionally reports the local-app mode as unsupported, while headless/API collection may still use explicit environment credentials.
...
DeepSeek uses `source: "api"` and has no rate-limit windows; its `windows` array carries only the balance as a `credits` window. OpenRouter, GLM/Z.ai, Volcengine, Qoder, WorkBuddy, Kimi, Ollama, and third-party adapters report quota/credit windows through the same `windows` array. Third-party adapter identifiers, profile Base URLs, endpoint paths, response mappings, user IDs, and raw credentials remain local and are never added to this wire shape.
`windows[].kind` is `session`, `weekly`, or `billing`. `windows[].source` is optionally `web` or `local` when one provider row combines components from different origins; OpenCode uses it so a device can hide its own opt-in local DB estimate without hiding Web quota or another device's estimate. Readers must treat the field as component provenance rather than replacing `limits.providers[].source`, and older windows may omit it. `windows[].metric` is an optional stable machine-readable role; `credits` identifies a provider's monetary balance/credits meter and `spend` a money-already-consumed meter, both independently of the display label (currently OpenRouter account credits, third-party account/token quota, DeepSeek balance, and MiMo balance). `windows[].displayRole: "balance"` marks an absolute balance presentation even when the unit is not money; WorkBuddy uses this with `windows[].unit: "credits"` because its remaining/limit values are points, not currency. A `credits` window's headline value is money, carried as an absolute `remaining` amount rather than a percentage; a `displayRole: "balance"` window uses the same absolute-value layout with its declared unit. Balance providers with no fixed quota denominator report no `usedPercent`/`remainingPercent` at all, and any meter percentage for them is derived by the renderer and deliberately kept off the wire. `windows[].currency` is an optional currency code (uppercase, at most 8 characters) that applies to a monetary window's absolute `used`/`limit`/`remaining` amounts, so a balance renders in its own currency without conversion. Normalization restores a `credits` window from a provider's `balance.amount` when the record carries a balance but no such window, so records posted by devices older than this field keep rendering; it does not synthesize a second window when a provider already carries `displayRole: "balance"`. A collector that reports a balance and does not want that synthesized meter must emit its own `credits` window with `showMeter: false`, as Claude does. Claude may additionally report a `billing` window with `metric: "spend"` labelled `Usage credits`, carrying `used` and — only when the account has set a monthly spend limit — `limit` and a derived `usedPercent`. Without such a limit it reports `used` alone with `showMeter: false`; the upstream `spend.percent` field reports `0` rather than `null` in that state and is deliberately not forwarded. `windows[].detail` is an optional bounded display-only description for a window, such as the Kimi-vs-Code composition of the single shared monthly membership meter; it must not contain credentials or raw provider response data.
