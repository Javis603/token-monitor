...
`periodWindows` is optional. Agents and widgets stamp each snapshot with the UTC instant its `today`/`month` windows end, computed in the device's own local time (`endsAt` = next local midnight / next local month start; `key` is the device-local day/month for reference). Current clients also include the optional IANA `timeZone`, allowing rolling ranges to advance in the device's calendar after an offline snapshot's original window has expired; hubs validate it before storing. The hub uses `endsAt` to expire a device's `today`/`month` from the aggregate once `now >= endsAt`, so a device that goes offline before re-posting does not keep contributing a stale day/month snapshot (`allTime` never expires). Payloads without `periodWindows` retain the aggregate's legacy UTC day/month fallback against `updatedAt`; calendar-derived ranges instead fail closed once a legacy window expires without an IANA timezone, because UTC cannot recover the device-local date reliably.

`periods.*.capabilities` is optional and additive. Current producers send `tokenComponents: true` when cache-read, cache-write, and output counters are exact, and `clientModels: true` when the client-to-model intersection is present. Missing flags from older producers remain false through Hub normalization; consumers preserve total tokens and cost but expose the unsplittable remainder as unavailable rather than upgrading absent component fields to exact zero.

`historyAvailable` is an optional device capability. Current agents send `true` when retained history is enabled and `false` when it is disabled; an explicit `history: null` from an older agent is treated as `false`. Hubs preserve the flag across interval ticks that omit the history payload. `historyOmitted: true` is a separate freshness state: it means the producer supports History, but the latest requested refresh could not publish complete v2 coverage, either because collection failed or because the payload exceeded the ingest budget. Sync first removes client→model attribution, then all client/model attribution while retaining exact daily totals/components and coverage; only a payload whose non-History data leaves no room for that compact form uses the omission state. The Hub may retain the last-good payload for recovery, but consumers must treat that device's derived ranges as unavailable until a complete history arrives and clears the marker. Consumers must not calculate a cross-device rolling or custom range when a device that contributes usage has unavailable history, because doing so would silently treat that device's unknown historical usage as zero.

Flexible ranges require `history.schemaVersion: 2` and `history.coverage`, whose inclusive device-local `start` / `end` date keys identify the complete retained daily window. Sparse daily rows inside it mean measured zero-use days; dates outside it are unknown and must fail closed. Coverage is emitted only by a successful complete graph scan and is never advanced by a carried archive, live period window, viewer date or requested range. Older History shapes remain valid for native `DAY` / `MONTH` / `TOTAL` data but are not eligible for `WEEK` / `7D` / `30D` / custom ranges. Consequently every participating app/agent and the Hub or Worker must be updated before flexible ranges become available in a multi-device setup.

`history.capabilities` is optional and additive. `tokenComponents: true` means every retained daily total includes exact cache-read, cache-write, and output counters. `attribution: true` means `perClient` and `perModel` are complete, while `clientModels: true` means their intersection is present. Consumers must treat an absent or false capability as unavailable detail rather than as a measured zero. Current histories additionally carry `unclassifiedTokens` on daily/monthly rows and their available `perClient` / `perModel` entries: it is the retained token total whose input/output/cache split is unavailable, allowing a mixed range to show its exact component subset without silently treating the remainder as cache misses. Version-1 local history archives are migrated without losing totals, but remain component-incomplete until their retained days are replaced by fresh graph observations. When a history approaches the sync budget, the producer may first remove `perClientModel` and set `clientModels: false`, then remove `perClient` / `perModel` and set `attribution: false`; exact daily totals and coverage continue to synchronize.
