...
For local API-equivalent estimates, periods may additionally carry `clientModelTokenComponents[client][model]` with exact row-level `{ input, output, cacheRead, cacheWrite, unclassified, complete }` counters. It is the only supported client × model × category cross-product. A producer may mark `complete: true` only when the row's total closes over those categories; consumers must not reconstruct it from period/model totals, allocate categories proportionally, or interpret `unclassified` as input. If cache + output + write exceed the row total, the whole row fails closed as unclassified rather than keeping known categories and also counting the total as unclassified. The field is additive through collector, device normalization, merge, delta, Hub, and Worker paths. Older payloads simply omit it and therefore remain unpriced by the Codex quota adapter's local API-equivalent accounting. If this additive cross-product would push a device payload over its ingest budget, serialization drops `clientModelTokenComponents` from each period that carried it, stamps that period with `clientModelTokenComponentsOmitted: true`, and also sets a device-level `clientModelTokenComponentsOmitted: true` for logs. The period flag is the ingest/aggregation authority: today's omit must not be copied onto month or allTime. A missing flag is not an omit — older payloads simply lack the map and stay on the existing capability/unpriced path. Codex quota capture fail-closes an explicitly omitted period instead of reconstructing remainder-as-input from aggregates.
