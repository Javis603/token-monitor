...
`historyAvailable` is an optional device capability. Current agents send `true` when retained history is enabled and `false` when it is disabled; an explicit `history: null` from an older agent is treated as `false`. Hubs preserve the flag across interval ticks that omit the history payload. `historyOmitted: true` is a separate transport state: it means the producer supports History, but its latest retained payload could not fit within the ingest budget. The Hub may retain the last-good payload for recovery, but consumers must treat that device's derived ranges as unavailable until a complete history arrives and clears the marker. Consumers must not calculate a cross-device rolling or custom range when a device that contributes usage has unavailable history, because doing so would silently treat that device's unknown historical usage as zero.

`history.coverage` is optional and contains inclusive device-local `start` / `end` date keys for the complete retained daily window. Range consumers validate every participating device against this interval; sparse daily rows inside it mean measured zero-use days, while dates outside it are unknown and must fail closed. Older histories without this metadata are interpreted using the legacy 370-day window anchored to the producer's `periodWindows.today.key`, never to the viewer's newer date.
