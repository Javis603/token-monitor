...
`limits.providers[].provider` is one of `claude`, `codex`, `factory`, `cursor`, `antigravity`, `opencode`, `openrouter`, `deepseek`, `minimax`, `mimo`, `grok`, `copilot`, `kiro`, `commandcode`, `zai`, `zaiteam`, `volcengine`, `qoder`, `workbuddy`, `kimi`, `ollama`, `alibaba`, or `thirdparty`. `factory` is the stable limits-provider id for the `droid` usage client; the product-facing label is Factory Droid.
