...
Periods may carry `unpricedTokens`, the number of tokens whose source could not resolve a complete price, with `clientUnpricedTokens`, `modelUnpricedTokens`, and `clientModelUnpricedTokens` maps preserving attribution. Sessions carry the scalar and `modelUnpricedTokens`; project rollups carry the scalar. History daily/monthly rows and the summary, plus daily `perClient` / `perModel` entries, also retain the scalar. Currently Qoder CN emits this metadata when its existing pricing lookup returns no price or lacks a rate for a used token component. `costUsd` and existing cost maps remain numeric known-cost subtotals: a positive missing-price count means the subtotal is incomplete, not that those tokens were free. Explicit zero rates remain valid prices. Missing metadata defaults to zero for compatibility with older producers; it does not retroactively establish pricing coverage for older records. Pricing sources, custom-pricing precedence, token totals, and provider Credits/balance reporting are unchanged.

Period normalization caps the missing-price scalar at `totalTokens` and each attribution count at its matching normalized token bucket. Entries without a matching usage bucket are discarded, and multiple raw keys that normalize to one bucket share that bucket's limit. Client-archive restoration applies the same scalar and model-bucket bounds without changing token totals or known costs.
