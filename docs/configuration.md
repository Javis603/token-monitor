...
| **Appearance** | Interface theme (presets such as Default and Obsidian, a porcelain light mode, or custom colors), per-vendor tool colors, system glass opacity / blur (on macOS 26+, native Liquid Glass or the classic frosted style), and separate interface and display fonts. |
