...
Provider credentials (Droid/Factory, Grok, DeepSeek, Minimax, Copilot, GLM / GLM Team, Volcengine, Qoder, Command Code, WorkBuddy, Ollama, Kimi, Alibaba Token Plan, …) and proxy settings live in the same file. **`.env.example` is the complete, authoritative list** — start from it rather than copying keys by hand, since it stays in sync with the code. Droid/Factory limits use `FACTORY_API_KEY` from Token Monitor settings, the process environment, or Droid's local `.env`; Droid OAuth files and system credential stores are not read. The desktop widget likewise reads the session owned by the local WorkBuddy app when that provider is enabled; the WorkBuddy token fields above remain only for headless/CLI deployments.
