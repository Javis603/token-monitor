...
test('a single sample carries no rate, so nothing is scheduled', () => {
  ...
  recordLimitsSample(state, limits, 0);
...
test('a burning window schedules an early refresh anchored to the last sample', () => {
  const state = createLimitsBurnState();
  recordLimitsSample(state, summary([sessionWindow(68)], { updatedAt: 'a' }), 0);
  const limits = summary([sessionWindow(88)], { updatedAt: 'b' });
  recordLimitsSample(state, limits, BASE_MS);
...
test('a delay above the floor is used verbatim', () => {
  const state = createLimitsBurnState();
  recordLimitsSample(state, summary([sessionWindow(60)], { updatedAt: 'a' }), 0);
...
test('a low but idle window schedules nothing', () => {
  const state = createLimitsBurnState();
  recordLimitsSample(state, summary([sessionWindow(85)], { updatedAt: 'a' }), 0);
...
test('a burning window with plenty of headroom schedules nothing', () => {
  const state = createLimitsBurnState();
  recordLimitsSample(state, summary([sessionWindow(0)], { updatedAt: 'a' }), 0);
...
test('an exhausted window schedules nothing', () => {
  const state = createLimitsBurnState();
  recordLimitsSample(state, summary([sessionWindow(80)], { updatedAt: 'a' }), 0);
...
test('credits windows are ignored', () => {
  ...
  recordLimitsSample(state, summary([credits(68)], { updatedAt: 'a' }), 0);
...
test('windows without a usable percentage are ignored', () => {
  ...
  recordLimitsSample(state, summary([bare(null)], { updatedAt: 'a' }), 0);
...
test('a window reset re-baselines instead of producing a negative rate', () => {
  ...
  const state = createLimitsBurnState();
  recordLimitsSample(state, summary([sessionWindow(90, { resetsAt: '2026-08-13T01:00:00.000Z' })], { updatedAt: 'a' }), 0);
  const reset = summary([sessionWindow(2, { resetsAt: '2026-08-13T06:00:00.000Z' })], { updatedAt: 'b' });
  recordLimitsSample(state, reset, BASE_MS);
  ...
  recordLimitsSample(state, after, BASE_MS * 2);
...
test('a percentage that drops without a reset stamp also re-baselines', () => {
  const state = createLimitsBurnState();
  recordLimitsSample(state, summary([sessionWindow(90)], { updatedAt: 'a' }), 0);
  const dropped = summary([sessionWindow(30)], { updatedAt: 'b' });
  recordLimitsSample(state, dropped, BASE_MS);
  assert.equal([...state.windows.values()][0].rate, 0);
  assert.equal(next(dropped, state, BASE_MS), null);

  recordLimitsSample(state, summary([sessionWindow(50)], { updatedAt: 'c' }), BASE_MS * 2);
...
test('the rate reacts at once to a faster burn and decays slowly when it stops', () => {
  ...
  recordLimitsSample(state, summary([sessionWindow(0)], { updatedAt: 'a' }), 0);
  recordLimitsSample(state, summary([sessionWindow(20)], { updatedAt: 'b' }), BASE_MS);
  const peak = key();
  assert.ok(peak > 0);

  recordLimitsSample(state, summary([sessionWindow(20)], { updatedAt: 'c' }), BASE_MS * 2);
  const once = key();
  recordLimitsSample(state, summary([sessionWindow(20)], { updatedAt: 'd' }), BASE_MS * 3);
  const twice = key();
  assert.ok(once < peak && once > 0, 'one quiet interval only decays the rate');
  assert.ok(twice < once && twice > 0, 'the decay continues');

  recordLimitsSample(state, summary([sessionWindow(40)], { updatedAt: 'e' }), BASE_MS * 4);
...
test('the most urgent window of a provider wins', () => {
  ...
  recordLimitsSample(state, summary(windows(60, 10), { updatedAt: 'a' }), 0);
...
test('the earliest provider wins and only its scope is returned', () => {
  ...
  recordLimitsSample(state, both(60, 60, 'a'), 0);
...
test('keys and scopes are returned in the same order', () => {
  ...
  recordLimitsSample(state, both(60, 'a'), 0);
