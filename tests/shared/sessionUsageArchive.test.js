...
test('captures and reapplies missing sessions for any client without double-counting live sessions', () => {
  ...
  assert.equal(visible.today.capabilities.tokenComponents, true);
  assert.equal(visible.today.unclassifiedTokens, 0);
  assert.equal(visible.today.inputTokens, 10);
  assert.equal(visible.today.clientInputs.opencode, 10);
  assert.equal(visible.today.modelInputs['claude-3-5-sonnet'], 10);
