...
function row(model, messageId, input = 10, output = 2) {
  return { sessionId: 's', messageId, model, input, output, cacheRead: 3, cacheWrite: 0,
    createdAt: Date.parse('2026-09-19T08:00:00Z'), messages: 1 };
}
...
test('Qoder missing pricing preserves known cost subtotal and unpriced token provenance', () => {
  ...
  const raw = buildQoderCnPeriods({ now: '2026-09-19T12:00:00Z', rows, pricingByModel }).today;
...
test('Qoder explicit zero prices remain free while an absent required rate remains unavailable', () => {
  const period = extractUsageFromTokscale(buildQoderCnPeriods({
    now: '2026-09-19T12:00:00Z', rows: [row('free', 'f'), row('incomplete', 'i')],
    pricingByModel: {
...
test('Qoder history and live archive round-trip unpriced tokens', () => {
  ...
  const period = extractUsageFromTokscale(buildQoderCnPeriods({ now: '2026-09-19T12:00:00Z', rows: [row('missing', 'u')], pricingByModel: {} }).today);
...
test('period delta clears stale unpriced provenance after pricing becomes available', () => {
  const missing = extractUsageFromTokscale(buildQoderCnPeriods({ now: '2026-09-19T12:00:00Z', rows: [row('m', 'u')], pricingByModel: {} }).today);
  const priced = extractUsageFromTokscale(buildQoderCnPeriods({ now: '2026-09-19T12:00:00Z', rows: [row('m', 'u')], pricingByModel: { m: {
