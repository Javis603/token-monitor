...
test('derived KNOWN_CLIENTS keeps the established id order', () => {
  ...
  assert.equal(
    KNOWN_CLIENTS,
    'claude,codex,opencode,hermes,openclaw,cursor,antigravity,cline,amp,droid,kimi,qwen,grok,copilot,pi,zed,kilo,commandcode,mimo,zcode,kiro,codebuddy,workbuddy,proma,qodercn,liveagent,reasonix,dsh,cherrystudio,lmstudio,unsloth,devin'
  );
});
...
test('derived DEFAULT_CLIENTS keeps the existing default-tracked CSV', () => {
  ...
  assert.equal(
    DEFAULT_CLIENTS,
    'claude,codex,opencode,hermes,openclaw,cursor,antigravity,cline,amp,droid,kimi,qwen,grok,copilot,pi,zed,kilo,commandcode,mimo,zcode,kiro,codebuddy,workbuddy,proma,liveagent,reasonix,dsh,cherrystudio,lmstudio,unsloth,devin'
  );
});

test('derived PARSE_LOCAL_CLIENTS still lists exactly the local adapters', () => {
  assert.deepEqual([...PARSE_LOCAL_CLIENTS], ['proma', 'qodercn', 'liveagent']);
  assert.deepEqual([...LOCALLY_PARSED_CLIENT_IDS], ['proma', 'qodercn', 'liveagent']);
});
