...
test('derived DEFAULT_CLIENTS keeps the existing default-tracked CSV', () => {
  ...
  assert.equal(
    DEFAULT_CLIENTS,
    'claude,codex,opencode,hermes,openclaw,cursor,antigravity,cline,amp,droid,kimi,qwen,grok,copilot,pi,zed,kilo,commandcode,mimo,zcode,kiro,codebuddy,workbuddy,proma,reasonix,dsh,cherrystudio,lmstudio,unsloth,devin'
  );
});

test('derived PARSE_LOCAL_CLIENTS still lists exactly the local adapters', () => {
  assert.deepEqual([...PARSE_LOCAL_CLIENTS], ['gcmp', 'proma', 'qodercn']);
  assert.deepEqual([...LOCALLY_PARSED_CLIENT_IDS], ['gcmp', 'proma', 'qodercn']);
});
