...
test('projectRowsForPeriod merges sessions by workspace and sorts by cost', () => {
  const rows = projectRowsForPeriod({ sessions: {
    a: { client: 'codex', projectId: 'sha256:a', projectLabel: 'client-a', totalTokens: 100, costUsd: 2 },
    b: { client: 'claude', projectId: 'sha256:a', projectLabel: 'client-a', totalTokens: 50, costUsd: 1 },
  ...
  assert.deepEqual({ key: rows[0].key, name: rows[0].name, value: rows[0].value, cost: rows[0].cost, detail: rows[0].detail }, { key: 'sha256:a', name: 'client-a', value: 150, cost: 3, detail: 'Claude Code, Codex' });
