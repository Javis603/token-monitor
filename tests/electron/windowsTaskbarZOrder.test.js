...
// Re-asserting costs a timer and a visible flicker on every app switch, so it
// only runs for someone who asked for it on the platform that needs it.
test('re-asserting is opt-in and Windows-only', () => {
  assert.equal(taskbarZOrderEnabled({ keepAboveTaskbar: true }, 'win32'), true);
  assert.equal(taskbarZOrderEnabled({ keepAboveTaskbar: false }, 'win32'), false);
  assert.equal(taskbarZOrderEnabled({}, 'win32'), false);
  assert.equal(taskbarZOrderEnabled(null, 'win32'), false);
  assert.equal(taskbarZOrderEnabled({ keepAboveTaskbar: true }, 'darwin'), false);
  assert.equal(taskbarZOrderEnabled({ keepAboveTaskbar: true }, 'linux'), false);
});

test('the setting only takes a literal true, never a truthy leftover', () => {
  for (const value of ['true', 1, 'yes', {}]) {
    assert.equal(taskbarZOrderEnabled({ keepAboveTaskbar: value }, 'win32'), false);
  }
});
