...
test('derived ranges drive headline and Home usage without pretending sessions or projects are available', () => {
  ...
  assert.match(app, /function currentUsagePeriod\(\)[\s\S]*usagePeriodForStats\(state\.stats\)/);
  assert.match(app, /function renderHome\(\)[\s\S]*const period = currentUsagePeriod\(\)/);
  assert.match(app, /function render\(\)[\s\S]*const period = currentUsagePeriod\(\)/);
  assert.match(app, /periodRangesApi\.supportsBreakdown\(selection, state\.breakdown\)/);
  assert.match(app, /detailUnavailable \|\| deviceHistoryStatus !== 'ready' \? \[\] : rowsForPeriod\(period\)/);
...
test('derived device rows load per-device histories instead of falling back to today', () => {
  ...
  assert.match(app, /deviceHistoriesLoadedSignature === signature/);
